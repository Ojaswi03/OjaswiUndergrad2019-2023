...
Main	LD R0,Input1	; Our functions assume R0 and R1 are set as preconditions,
	...
	LEA R3,Functions; R3 pointer to array of functions.
	ADD R2,R2,R3	; R2 now points to a specific function
	LDR R2, R2, #0	; R2 now holds the address of desired function 
	JSRR R2		; JSRR allows us to do a variable subroutine call, whereas
			; JSR always calls the same function.
	ST R0,Output	; R0 is always the solution.
End_Main	HALT
